...
import 'package:tarot/screens/handbook_card_description.dart';
...
class SingleCategoryCardsGrid extends StatefulWidget with PlanetScreenMixin {
  ...
  _SingleCategoryCardsGridState createState() => _SingleCategoryCardsGridState(
      AdManager.singleCategoryGridInterstitialAdUnitId);
...
class _SingleCategoryCardsGridState
    extends BaseAdScreenState<SingleCategoryCardsGrid> {
  ScrollController _controller = ScrollController();
  List<TarotCard> _cards = [];
  int _cardIndexOffset = 0;

  _SingleCategoryCardsGridState(String interstitialAdUnitId)
        ...
        child: AnimatedBuilder(
          ...
          builder: (context, child) => AnimatedOpacity(
            opacity: _controller.hasClients
                ? (1.0 - _controller.offset / 50.0).clamp(0.0, 1.0).toDouble()
                : 1.0,
        ...
        child: GridView.builder(
          ...
          itemBuilder: (context, index) {
            return CardGridItem(
              ...
              imgAsset: CardFacesDirectory.cardFaceDirectory() +
                  _cards[index].imgAsset,
              onTap: () {
                _onTap(index);
              },
            );
...
class CardGridItem extends StatelessWidget {
  ...
  const CardGridItem(
      {Key? key, required this.imgAsset, required this.title, this.onTap})
      : super(key: key);
  @override
  Widget build(BuildContext context) {
